...
const network = Network[networkStr] || Network.ETH_MAINNET;
...
const isValidGetNftsForOwnerParams = (args) => {
...
const isValidGetNftMetadataParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
...
const isValidGetTransactionReceiptsParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
...
const isValidGetTokenMetadataParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
        typeof args.contractAddress === "string");
};
const isValidGetTokensForOwnerParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
        typeof args.owner === "string" &&
        (args.pageKey === undefined || typeof args.pageKey === "string") &&
...
const isValidGetNftsForContractParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
        typeof args.contractAddress === "string" &&
        (args.pageKey === undefined || typeof args.pageKey === "string") &&
...
const isValidGetBlockWithTransactionsParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
...
const isValidGetTransactionParams = (args) => {
    return (typeof args === "object" && args !== null && typeof args.hash === "string");
};
const isValidResolveEnsParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
...
const isValidLookupAddressParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
        typeof args.address === "string");
};
const isValidEstimateGasPriceParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
        (args.maxFeePerGas === undefined || typeof args.maxFeePerGas === "boolean"));
};
const isValidSubscribeParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
...
const isValidUnsubscribeParams = (args) => {
    return (typeof args === "object" &&
        args !== null &&
...
export class AlchemyMcpServer {
                    ...
                    inputSchema: {
                        ...
                        properties: {
                            ...
                            category: {
                                ...
                                description: 'The category of transfers to include (e.g., "external", "internal", "erc20", "erc721", "erc1155", "specialnft")',
                            },
                            contractAddresses: {
                                type: "array",
                                items: {
    ...
    validateAndCastParams(args, validator, errorMessage) {
        if (!validator(args)) {
            throw new McpError(ErrorCode.InvalidParams, errorMessage);
        }
        return args;
    }
    isValidEstimateGasPriceParams = (args) => {
        return (typeof args === "object" &&
            args !== null &&
            (args.maxFeePerGas === undefined ||
                typeof args.maxFeePerGas === "boolean"));
    };
    isValidSubscribeParams = (args) => {
        return (typeof args === "object" &&
            args !== null &&
            typeof args.type === "string" &&
            ["newHeads", "logs", "pendingTransactions", "mined"].includes(args.type) &&
            (args.address === undefined || typeof args.address === "string") &&
            (args.topics === undefined || Array.isArray(args.topics)));
    };
    isValidUnsubscribeParams = (args) => {
        return (typeof args === "object" &&
            args !== null &&
            typeof args.subscriptionId === "string");
    };
    // Then in your AlchemyMcpServer class, make sure these handlers are included:
    async handleEstimateGasPrice(args) {
        const params = this.validateAndCastParams(args, isValidEstimateGasPriceParams, "Invalid gas price parameters");
    ...
    async handleSubscribe(args) {
        const params = this.validateAndCastParams(args, isValidSubscribeParams, "Invalid subscribe parameters");
        ...
        switch (params.type) {
            ...
            case "mined":
                ...
                break;
            default:
                throw new McpError(ErrorCode.InvalidParams, `Unknown subscription type: ${params.type}`);
        }
        this.activeSubscriptions.set(subscriptionId, subscription);
        return { subscriptionId };
    }
    async handleUnsubscribe(args) {
        const params = this.validateAndCastParams(args, isValidUnsubscribeParams, "Invalid unsubscribe parameters");
        const subscription = this.activeSubscriptions.get(params.subscriptionId);
        if (!subscription) {
            throw new McpError(ErrorCode.InvalidParams, `Subscription not found: ${params.subscriptionId}`);
        }
        subscription.unsubscribe();
        this.activeSubscriptions.delete(params.subscriptionId);
        return { success: true };
    }
    async handleGetNftsForOwner(args) {
        const params = this.validateAndCastParams(args, isValidGetNftsForOwnerParams, "Invalid NFTs for owner parameters");
        return await this.alchemy.nft.getNftsForOwner(params.owner, params);
    }
    async handleGetNftMetadata(args) {
        const params = this.validateAndCastParams(args, isValidGetNftMetadataParams, "Invalid NFT metadata parameters");
        return await this.alchemy.nft.getNftMetadata(params.contractAddress, params.tokenId, params);
    }
    async start() {
        try {
